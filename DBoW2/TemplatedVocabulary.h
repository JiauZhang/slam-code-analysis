  ...
  BowVector &v, FeatureVector &fv, int levelsup) const
{
  v.clear();
  fv.clear();
  // empty() 检查 Words of the vocabulary (tree leaves)是否为空
  ...
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    ...
    //为所有的特征描述子计算BoWVec和FeatureVec，它俩不同层
    //BoWVce一直到叶子节点，而FeatureVec仅到有限层
    for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
    {
      ...
      { 
        //word_id, weight
        v.addWeight(id, w);
        //node_id, i_feature(关键点的索引值index)
...
{ 
  ...
  do
  {
    ...
    //feature id 只到 nid_level，并不会像描述子一样，一直算到叶节点才停止
