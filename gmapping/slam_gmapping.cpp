...
void SlamGMapping::init()
{
  ...
  if(!private_nh_.getParam("throttle_scans", throttle_scans_))
    throttle_scans_ = 1;
  // 基坐标系，可参考 http://wiki.ros.org/navigation/Tutorials/RobotSetup/TF
  // 也即移动机器人的载体坐标系 http://www.ros.org/reps/rep-0105.html
  if(!private_nh_.getParam("base_frame", base_frame_))
    base_frame_ = "base_link";
  // 世界坐标系
  if(!private_nh_.getParam("map_frame", map_frame_))
    map_frame_ = "map";
  // odom 指的是世界坐标系下固定的坐标系，可以看做移动其机器人
  // 运动开始的原点处 http://www.ros.org/reps/rep-0105.html
...
// gmap_pose 最终记录的是里程计下的 (x, y, theta)
bool SlamGMapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t)
{
  ...
  {
  	// 根据时间戳在里程计坐标系下取出位姿
  	// centered_laser_pose_ 记录的是激光扫描扇区中线的方位角
  	// 会在初始化的时候给出
  	// odom_frame 为输入, centered_laser_pose_ 为输入, odom_pose 为输出
  	// 即激光与机器人的方位角以 centered_laser_pose_ 的朝向为准
  	// 由于里程计自身会不断地估算基坐标的位姿，所以叠加之后就可以得到
  	// 激光在里程计下的位姿了
...
// 1. 检查基坐标系和激光坐标系 z轴是否对齐
// 2. 设置激光参数等，初始化粒子群 m_particles 、高斯随机种子
bool SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
  ...
  // http://docs.ros.org/jade/api/tf/html/c++/classtf_1_1Transformer.html#a5de45f874d754450b9d4d5760830f404
  // 将激光扫描转换到基坐标系下
  // 例程可参考 http://wiki.ros.org/navigation/Tutorials/RobotSetup/TF
  try
  {
  	// 获取 laser 在 基坐标系下的位姿
  ...
  // gmapping 不关心 滚转 和 俯仰
  // 判断在基坐标系下在上方 1m，是否在laser下也成立，如果成立则说明它们的 z轴 是平行的
  // 至于绝对值说明允许 z 轴方向相反，但是必须平行
  if (fabs(fabs(up.z()) - 1) > 0.001)
  {
	...
	// 有效测距数量 http://docs.ros.org/kinetic/api/sensor_msgs/html/msg/LaserScan.html
  gsp_laser_beam_count_ = scan.ranges.size();
  // 扫描扇形中线
  double angle_center = (scan.angle_min + scan.angle_max)/2;
  // 大于 0 说明 z 轴同向，即向上
  // 激光坐标系：x轴向前、z轴向上、y轴向右
  if (up.z() > 0)
  {
    do_reverse_range_ = scan.angle_min > scan.angle_max;
	// 激光扫描的扇形中线的旋转姿态，半角、零平移
  ...
  // gmapping 使用 正的角度增量，如果实际情况是负的，则反转数据
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  ...
  ROS_DEBUG("processing scan");
	// 默认参数：processScan(const RangeReading & reading, int adaptParticles=0)
