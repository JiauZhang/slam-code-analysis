...
void LocalMapping::Run()
{
    ...
    while(ros::ok())
    {
        ...
        {            
            ...
            //1.取出新的关键帧，计算描述子的BoW
            //2.更新该关键帧中记录的MapPoint的观测、平均单位向量、最大距离和最小距离及最佳描述子
            //3.更新共视图
            //4.将此关键帧添加到Map中
            ProcessNewKeyFrame();

            // Check recent MapPoints
            //删除mlpRecentAddedMapPoints中一些不好的MapPoint
            MapPointCulling();

            // Triangulate new MapPoints
            //1.取当前关键帧一定数量的共视关键帧
            //2.找出当前帧中与共视帧中没有匹配的MapPoint
            //3.通过三角测量产生新的MapPoint
            CreateNewMapPoints();
            ...
            mbAbortBA = false;
            //如果没有新的关键帧到来，且没有停止请求，则进行LocalBundleAdjustment
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                //遍历当前关键帧的所有共视关键帧，并计算冗余度，冗余度>0.9就标记mbToBeErased=true
                KeyFrameCulling();
...
void LocalMapping::ProcessNewKeyFrame()
{
        ...
        for(size_t i=0; i<vpMapPointMatches.size(); i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    //1.计算所有观测到该MapPoint的相机到MapPoint的单位向量的平均和
                    //2.计算MapPoint被观测某帧到时，可能与相机的最大距离和最小距离
    ...
    {
        //如果取出的关键帧为第一帧，将当前帧匹配到的MapPoint都保存到mlpRecentAddedMapPoints中
        for(size_t i=0; i<vpMapPointMatches.size(); i++)
        {
            MapPoint* pMP = vpMapPointMatches[i];
            if(pMP)
    ...
    //更新当前帧共视图，当共视数量超过阈值时，将其添加到共视关键帧中
    //并把与其共视最多的作为自己的父节点，且添加到父节点的子节点数据结构中
...
void LocalMapping::MapPointCulling()
{
    ...
    //遍历最近刚刚添加的所有MapPoint，删除一些不好的MapPoint
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        if(pMP->isBad())
        {
            //从mlpRecentAddedMapPoints删除
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
        //被观测到次数与可观测到次数的比例过低，删除
        else if(pMP->GetFoundRatio()<0.25f )
        {
            //设置mbBad=true，清除关键帧对其的记录，并从Map中删除
            pMP->SetBadFlag();
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
        //如果该MapPoint所属的关键帧距当前帧>=2个关键帧且观测到它的关键帧还<=2，则删除
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        //返回pKF2中MapPoint距离关键帧的距离的中间值
        const float medianDepthKF2 = pKF2->ComputeSceneMedianDepth(2);
        //两个关键帧之间基线与pKF2中间深度值的比值，越小说明视差越小，三角测量越不可靠
...
//1.取出一定数量的当前关键帧的共视关键帧
//2.将当前关键帧的所有匹配的MapPoint分别投影到共视关键帧中寻找匹配点
//3.记录第一步中所有关键帧中未与当前融合的MapPoint，投影到当前关键帧中寻找匹配点
//4.更新共视图
void LocalMapping::SearchInNeighbors()
{
    ...
    vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();
    //将当前帧的MapPoint投影到vpTargetKFs中寻找匹配点
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        KeyFrame* pKFi = *vit;
        //将当前关键帧的MapPoint重投影到pKFi上，如果能够找到匹配点，则将当前帧的MapPoint也添加到pKFi中
    ...
    //将vpTargetKFs中没有跟当前帧融合的都记录下来，准备融合
    ...
    //将上一步记录的没有与当前帧融合的MapPoint投影到当前帧寻找匹配点
...
//1.取出当前关键帧的全部共视关键帧
//2.遍历所有共视关键帧，并计算冗余度，冗余度>0.9就标记mbToBeErased=true
void LocalMapping::KeyFrameCulling()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
                ...
                if(!pMP->isBad())
                {
                    ...
                    if(pMP->Observations()>3)
                    {
                        ...
                        for(map<KeyFrame*, size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
                            ...
                            //这里认为越接近原始图层看到越好，例如在原始图像上检测到比在金字塔图中检测到好
        ...
        //如果冗余比大于0.9，则设置此关键帧mbToBeErased=true
