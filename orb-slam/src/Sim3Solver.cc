    ...
    mnIterations(0), mnBestInliers(0)
{
    ...
    //SetRansacParameters(double probability = 0.99, int minInliers = 6 , int maxIterations = 300)
    SetRansacParameters();
}

void Sim3Solver::SetRansacParameters(double probability, int minInliers, int maxIterations)
{
    //default: probability = 0.99, minInliers = 6 , maxIterations = 300
...
cv::Mat Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
{
    ...
    while(mnIterations<mRansacMaxIts && nCurrentIterations<nIterations)
    {
        ...
        if(mnInliersi>=mnBestInliers)
        {
            ...
            mBestScale = ms12i;
            //如果inliers>最小inliers要求，则直接结束RANSAC
